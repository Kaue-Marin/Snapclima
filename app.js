//interação
const citySearchInput = document.getElementById("search-input");
const citySearchButton = document.getElementById("search-button");

//exibição
const dataAtual = document.getElementById("data-atual");
const nomeCidade = document.getElementById("nome-cidade");
const iconLogin = document.getElementById("icon-login");
const rainDice = document.getElementById("rain-dice");
const dataDegrees = document.getElementById("data-degrees");
const windSpeed = document.getElementById("wind-speed");
const feelsLikeTemperature = document.getElementById("feels-like-temperature");
const humidit = document.getElementById("humidit");
const sunriseTime = document.getElementById("sunrise-time");
const sunsetTime = document.getElementById("sunset-time");

const api_key = "c88db53e629d083c23b5d6ef06fe6f80";

// https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&lang=pt_br&appid=${api_key}
//     <!--c88db53e629d083c23b5d6ef06fe6f80-->

citySearchButton.addEventListener("click", () => {
  let cityName = citySearchInput.value;
  getCityWeather(cityName);
});

navigator.geolocation.getCurrentPosition(
  (position) => {
    let lat = position.coords.latitude;
    let lon = position.coords.longitude;
    getCurrentLocationWeather(lat, lon);
  },
  (err) => {
    if (err.code === 1) {
      alert(
        "Geolocalização negada pelo usuário, busque manualmente por uma cidade através da barra de pesquisa. "
      );
    } else {
      console.log(err);
    }
  }
);

function getCurrentLocationWeather(lat, lon) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&lang=pt_br&appid=${api_key}`
  )
    .then((response) => response.json())
    .then((data) => displayWeather(data));
}

function getCityWeather(cityName) {
  iconLogin.src = `./img/assets/loading-icon.svg`;

  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&units=metric&lang=pt_br&appid=${api_key}`
  )
    .then((response) => response.json())
    .then((data) => displayWeather(data));
}

function displayWeather(data) {
  let {
    dt,
    name,
    weather: [{ icon, description }],
    main: { temp, feels_like, humidity },
    wind: { speed },
    sys: { sunrise, sunset },
  } = data;

  dataAtual.textContent = formateDate(dt);
  nomeCidade.textContent = name;
  iconLogin.src = `./img/assets/${icon}.svg`;
  rainDice.textContent = description;
  dataDegrees.textContent = `${Math.round(temp)}°C`;
  windSpeed.textContent = `${Math.round(speed * 3.6)}km`;
  feelsLikeTemperature.textContent = `${Math.round(feels_like)}°C`;
  humidit.textContent = `${humidity}%`;
  sunriseTime.textContent = formatTime(sunrise);
  sunsetTime.textContent = formatTime(sunset);
}

function formateDate(epochTime) {
  let date = new Date(epochTime * 1000);
  let formateDate = date.toLocaleDateString("pt-BR", {
    month: "long",
    day: "numeric",
  });

  return `hoje, ${formateDate}`;
}

function formatTime(epochTime) {
  let date = new Date(epochTime * 1000);
  let hours = date.getHours();
  let minutes = date.getMinutes();
  return `${hours} : ${minutes}`;
}
